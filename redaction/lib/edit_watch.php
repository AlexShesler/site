<?php
    ...
    if (!empty($_POST['id1']) && !empty($_POST['title1']) && !empty($_POST['article1']) && !empty($_POST['price1']) && !empty($_POST['mechanism1']) && !empty($_POST['strap_material1']) && !empty($_POST['body_material1'])){
        ...
        $mechanism = htmlspecialchars(trim($_POST['mechanism1']));
        $strap_material = htmlspecialchars(trim($_POST['strap_material1']));
        $body_material = htmlspecialchars(trim($_POST['body_material1']));
    ...
    if (isset($_POST['btn_edit_watches']) && $result == true){
        $query = "UPDATE `".$table."s` SET title = '$title', article = '$article', price = '$price', mechanism = '$mechanism', strap_material = '$strap_material', body_material = '$body_material', description = '$description', discount = '$discount', new = '$new', showround = '$showround' WHERE id = '$id'";
    }
